...
namespace Orders.UnitTests.WebApi
{
    public class Orders
    {
        ...
        public async Task ClaimsController_Get_Calls_GetClaims_Query_And_Returns_404_If_Not_Found_Result()
        {
            ...
            mockMediator.Setup(mock => mock.Send(It.IsAny<GetOrders.Query>(), It.IsAny<CancellationToken>()))
                .Throws<NotFoundException>();
            var sut = new OrdersController(mockMediator.Object, mockLogger.Object, mockMapper.Object);
            var result = await sut.Get();

            mockMediator.Verify(mock => mock.Send(It.IsAny<GetOrders.Query>(), It.IsAny<CancellationToken>()));
        ...
        public async Task ClaimsController_Get_Calls_GetClaims_Query_And_Returns_OkObjectResult()
        {
            ...
            var mockLogger = new Mock<ILogger<OrdersController>>();
            var mockMapper = new Mock<IMapper>();
            var claimList = new List<OrderViewModel> {new()};
        ...
        public async Task ClaimsController_GetById_Calls_GetById_Query_And_Returns_404_If_Not_Found_Result()
        {
            ...
            var mockLogger = new Mock<ILogger<OrdersController>>();
            var mockMapper = new Mock<IMapper>();
            mockMediator.Setup(mock => mock.Send(It.IsAny<GetOrder.Query>(), It.IsAny<CancellationToken>()))
                .Throws<NotFoundException>();

            var sut = new OrdersController(mockMediator.Object, mockLogger.Object, mockMapper.Object);
            var result = await sut.GetById(1);
            mockMediator.Verify(mock => mock.Send(It.IsAny<GetOrder.Query>(), It.IsAny<CancellationToken>()));
            Assert.IsInstanceOf<NotFoundObjectResult>(result.Result);
        }
        ...
        [Test]
        public async Task
            ClaimsController_Post_Calls_CreateClaim_Command_And_Returns_BadRequest_On_ValidationException()
        {
            var mockMediator = new Mock<IMediator>();
            var mockLogger = new Mock<ILogger<OrdersController>>();
            var mockMapper = new Mock<IMapper>();
